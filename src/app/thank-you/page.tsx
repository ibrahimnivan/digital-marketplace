...
const ThankYouPage = async ({
  ...
  const orderId = searchParams.orderId  // order id in params
  const nextCookies = cookies()

  const { user } = await getServerSideUser(nextCookies) // only the one who paid can download
  ...
  const [order] = orders  // just taking first index

  if (!order) return notFound() // throw 404

  const orderUserId =
    typeof order.user === 'string'  // id = string
      ? order.user
      : order.user.id

  if (orderUserId !== user?.id) {  // unauthorized
    return redirect(
